...
Run:
    python comparisons/paper_figures_comparison.py \
...
CTRL_DISPLAY = {
    'Grid-Based HJ': 'Grid-Based HJ',
    'BRAT':          'BRAT (ours)',
    'MPC+Terminal':  'MPC+Terminal (ours)',
    'MPC':           'MPC',
    'Vanilla BRAT':  'Vanilla BRAT',
    'RL (DDQN)':     'RL (DDQN)',
}

# Fixed ordering: ground truth → your method → baselines by performance
CTRL_ORDER = ['Grid-Based HJ', 'BRAT', 'MPC+Terminal', 'MPC', 'Vanilla BRAT', 'RL (DDQN)']
...
# ═════════════════════════════════════════════════════════════════════════════
#  Figure 1 — 2×2 Metrics Bar Chart
# ═════════════════════════════════════════════════════════════════════════════

def plot_metrics_comparison(data, output_dir):
    """2×2 grouped bar chart of controller metrics (Fig. 4 style).

    Panels:
      (a) Outcome rates — docking, failure, timeout per controller
      (b) Mean control effort ± std (docking trajectories only)
      (c) Mean computation wall time ± std
      (d) Mean docking time ± std (docking trajectories only)
    """
    ...
    ax_rates.bar(x - w, dock_pct, w, color='#2ca02c', label='Docking', zorder=3)
    ax_rates.bar(x,     fail_pct, w, color='#d62728', label='Failure',  zorder=3)
    ax_rates.bar(x + w, time_pct, w, color='#b3b3b3', label='Timeout',  zorder=3)

    # Value labels on top of each bar group (docking rate only — most important)
    for i, v in enumerate(dock_pct):
        ax_rates.text(i - w, v + 1.0, f'{v:.0f}%', ha='center', va='bottom',
                      fontsize=7, color='#2ca02c')

    ax_rates.set_xticks(x)
    ax_rates.set_xticklabels(labels, rotation=35, ha='right')
    ...
    bars_e = ax_effort.bar(x, means_e, 0.55, yerr=stds_e, color=colors,
                           capsize=4, error_kw={'linewidth': 0.8}, zorder=3)
    for i, c in enumerate(ctrls):
        if data[c]['n_docking_effort'] == 0:
            bars_e[i].set_hatch('///')
            bars_e[i].set_facecolor('#e0e0e0')
            bars_e[i].set_edgecolor('#999999')
            ax_effort.text(i, 5, 'N/A', ha='center', va='bottom',
                           fontsize=8, color='#999999', fontweight='bold')
        else:
            n = data[c]['n_docking_effort']
            ax_effort.text(i, means_e[i] + stds_e[i] + 5, f'n={n}',
                           ha='center', va='bottom', fontsize=7, color='#555555')

    ax_effort.set_xticks(x)
    ax_effort.set_xticklabels(labels, rotation=35, ha='right')
    ...
    ax_wall.bar(x, means_w, 0.55, yerr=stds_w, color=colors,
                capsize=4, error_kw={'linewidth': 0.8}, zorder=3)
    ax_wall.set_xticks(x)
    ax_wall.set_xticklabels(labels, rotation=35, ha='right')
    ax_wall.set_ylabel('Wall Time (s)')
    ax_wall.set_title('Mean Computation Time per Rollout')
    ...
    bars_d = ax_dock.bar(x, means_d, 0.55, yerr=stds_d, color=colors,
                         capsize=4, error_kw={'linewidth': 0.8}, zorder=3)
    for i, c in enumerate(ctrls):
        if data[c]['n_docking_effort'] == 0:
            bars_d[i].set_hatch('///')
            bars_d[i].set_facecolor('#e0e0e0')
            bars_d[i].set_edgecolor('#999999')
            ax_dock.text(i, 0.5, 'N/A', ha='center', va='bottom',
                         fontsize=8, color='#999999', fontweight='bold')

    ax_dock.set_xticks(x)
    ax_dock.set_xticklabels(labels, rotation=35, ha='right')
    ...
    fig.savefig(os.path.join(output_dir, 'controller_comparison_6d.pdf'))
    fig.savefig(os.path.join(output_dir, 'controller_comparison_6d.png'))
    plt.close(fig)
    print(f"Saved controller_comparison_6d.{{pdf,png}} → {output_dir}")
