...
class MPC:
    ...
    def get_batch_data(self, initial_condition_tensor, T, policy=None, t=0.0,
        ...
        for i in range(num_iters):
            ...
            if self.dynamics_.set_mode in ['avoid', 'reach']:
                ...
                valid_idx=(min_idx>i).nonzero(as_tuple=True)
                value_labels_i =  torch.min(torch.clamp(reach_values[valid_idx[0], i:], min=torch.max(-avoid_values[valid_idx[0], i:], dim=-1).values.unsqueeze(-1)),dim=-1).values
                coord_i=coord_i[valid_idx]
            else:
                raise NotImplementedError
            # add to data
        ...
        if self.style == 'direct':
            ...
            if policy is not None:
                ...
                for i in range(self.num_effective_horizon_refinement):
                    self.warm_start_with_policy(initial_condition_tensor, policy, t) # optimize on the effective horizon first
            ...
            effective_refinement = self.num_effective_horizon_refinement
            self.num_effective_horizon_refinement = 0  # Reset so receding steps use full iterations
            ...
            #     
            self.num_effective_horizon_refinement = effective_refinement
    ...
    def warm_start_with_policy(self, initial_condition_tensor, policy=None, t_remaining=None):
        ...
        if self.incremental_horizon>0:
            ...
            state_trajs_H, permuted_controls_H = self.rollout_dynamics(initial_condition_tensor,start_iter=0, rollout_horizon=self.incremental_horizon)
    ...
    def rollout_dynamics(self, initial_state_tensor, start_iter, rollout_horizon, eps_var_factor=1):
        ...
        if self.sample_mode == "gaussian":
            ...
            permuted_controls [:, 0, ...] = self.control_tensors[:,start_iter:start_iter+rollout_horizon,:]*1.0 # always include the nominal trajectory
