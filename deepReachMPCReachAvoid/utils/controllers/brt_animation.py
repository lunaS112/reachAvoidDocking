...
def create_mpc_terminal_animation(controller, sim_result, save_path,
    ...
    ax.set_xlabel('x (m)', fontsize=12)
    ax.set_ylabel('y (m)', fontsize=12)
    ax.set_title('MPC + Terminal Cost Docking Control', fontsize=14)
    ...
    # Compute colourbar range
    if show_value_function:
        print("Computing value function range...")
        sample_values = []
        for t in [tMax, tMax / 2, tMax / 4, 0.1]:
            for i in range(0, resolution, 5):
                for j in range(0, resolution, 5):
                    state = np.array([X[i, j], Y[i, j], 0, 0, np.pi / 2, 0])
                    try:
    ...
    def animate(k):
        ...
        if cur_phase == 1:
            phase_text.set_text(
                f'Phase 1: Approach (MPC+Terminal)\n'
                f'Using V(x, tMax={tMax:.1f}s)')
            phase_text.set_bbox(
                dict(boxstyle='round', facecolor='lightblue', alpha=0.8))
        else:
            phase_text.set_text(
                f'Phase 2: Tracking (MPC+Terminal)\n'
                f't_remaining = {cur_t_rem:.2f}s')
            phase_text.set_bbox(
                dict(boxstyle='round', facecolor='lightgreen', alpha=0.8))
        ...
        if show_value_function:
            ...
            V = compute_value_grid(t_query, cur_vx, cur_vy,
                                   cur_theta, cur_omega)
            ...
            try:
                ...
                if colorbar_info['sm'] is not None:
                    ...
                    colorbar_info['cbar'].update_normal(colorbar_info['sm'])
                    colorbar_info['cbar'].set_label(
                        f'V(x, t={t_query:.1f}s)', fontsize=11)
            except Exception:
                pass
    ...
    # ---- Render ----
    print(f"Creating MPC+Terminal animation with {len(times)} frames "
          f"(skip={skip_frames})...")
    frames = range(0, len(times), skip_frames)
